...
function New_PNLE_entier_avec_pena_v1(usines, fournisseurs, emballages, d, cstop, ccam, γ, L, J, U, F, E, ind_J, ind_E, s0_u, s0_f, notrelaxed = true)
    # On résout le PNLE avec K camions disponibles par jour de manière optimale
    
    # On récupère les données liées à notre instance ou cluster
    b⁺, b⁻, r_u, r_f, cs_u, cs_f, cexc = data(usines, fournisseurs, emballages, ind_E, U, F, ind_J)
    
    model = Model(optimizer_with_attributes(Gurobi.Optimizer,"TimeLimit"=>200,"OutputFlag"=>1))
    @variable(model, q[1:J, 1:U, 1:F, 1:E] >= 0, integer = notrelaxed)

    #variables binaires pour la positivité des flux
    @variable(model, qpos[1:J, 1:U, 1:F] >= 0, Bin)
    ...
    # Contraintes sur qpos
    @constraint(model, [f in 1:F, u in 1:U, j in 2:J], qpos[j,u,f]*10000>=sum(q[j,u,f,e] for e in 1:E))
    ...
    @objective(model, Min, sum(qpos[j,u,f]*(ccam÷3+cstop+(γ*d[u,f])÷3) for u in 1:U, j in 1:J, f in 1:F) + sum(cost_su[e, u, j]*cs_u[e, u] for e in 1:E, u in 1:U, j in 1:J) + sum(cost_sf[e, f, j]*cs_f[e, f] for e in 1:E, f in 1:F, j in 1:J) + sum(neg[e, f, j]*cexc[e, f] for e in 1:E, f in 1:F, j in 1:J))
...
function New_PNLE_entier_avec_pena(usines, fournisseurs, emballages, d, cstop, ccam, γ, L, J, U, F, E, ind_J, ind_E, s0_u, s0_f, notrelaxed = true)
    # On résout le PNLE avec K camions disponibles par jour de manière optimale
